              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium
                    ${bus.checkInStatus ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}
                >
                  {bus.checkInStatus ? '탑승 완료' : '탑승 대기'}
                </span>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.destinations.map((dest) => (
            <div key={dest.id} className="bg-white rounded-lg shadow-sm p-4">
              ...
              <p className="text-sm text-gray-600 mb-4">{dest.time}</p>
              <div className="flex justify-between text-sm text-gray-600 mb-2">
