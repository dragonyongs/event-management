...
const GolfManagement = () => {
    ...
    const golfSubEvent = state.selectedEvent.subEvents.find(
        (sub) => sub.type === 'golf'
    ) || { groups: [] };

    const users = state.selectedEvent.users;
    const userMap = new Map(users.map(user => [user.id, user.name]));

    const onAddGroup = () => {
        console.log('🔵 onAddGroup 실행됨');
        dispatch({
            type: 'OPEN_DRAWER',
            drawer: 'isGroupDrawerOpen',
            payload: { drawerContent: 'golf' },
        });
    };
    ...
    const onEditGroup = (group) => {
        dispatch({
            type: 'OPEN_DRAWER',
            drawer: 'isGroupDrawerOpen',
            payload: { drawerContent: 'golf', selectedGroup: group },
        });
    };
    ...
    const onDropMember = (memberId, fromGroupId, toGroupId) => {
        dispatch({
            type: 'DROP_MEMBER',
            payload: { memberId, fromGroupId, toGroupId },
        });
        
    };


    // 회원의 상태에 따른 스타일을 반환하는 헬퍼 함수입니다.
    ...
    return (
        ...
        {golfSubEvent.groups.length === 0 ? (
            <div className="flex items-center justify-center h-96 rounded-2xl bg-white text-gray-500 text-center">
            추가된 조가 없습니다.
            </div>
        ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {golfSubEvent.groups.map((group) => (
